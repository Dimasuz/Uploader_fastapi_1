...
class TestRole:
    def test_get_roles(self, admin_client: FileApiClient):
        response = admin_client.get_roles()
        assert response.page == 1
        assert response.total == 2
        assert len(response.roles) >= 2

    def test_get_role(self, admin_client: FileApiClient):
    ...
    def test_create_role(self, admin_client: FileApiClient):
        response = None
        try:
    ...
    def test_update_role(self, admin_client: FileApiClient):
