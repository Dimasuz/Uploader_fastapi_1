...
class TestRight:
    def test_get_rights(self, admin_client: FileApiClient):
        response = admin_client.get_rights()
        assert response.page == 1
        assert response.total == 12
        assert len(response.rights) >= 0

    def test_get_rights_small_page(self, admin_client: FileApiClient):
    ...
    def test_create_right(self, admin_client: FileApiClient):
        response = None
        try:
    ...
    def test_delete_right(self, admin_client: FileApiClient):
        response = admin_client.create_right("token", True, False, False)
    ...
    def test_non_priveleged_user(self, client: FileApiClient, admin_client: FileApiClient):
